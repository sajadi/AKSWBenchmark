package org.benchmark.clustering;
...
public class ClusterProcessor {
    ...
    private static Logger logger = Logger.getLogger(ClusterProcessor.class);
    /**
    ...
    public static void sortQueries(){
        try{
            ...
            Collection files = FileUtilities.iterateThroughFiles(BenchmarkConfigReader.queryLogFolder, new String[]{"log"});
            ...
            File sortedQueriesFile = new File(BenchmarkConfigReader.queryLogFolder + "/sortedqueries.log");
            File sortedQueryOccurrencesFile = new File(BenchmarkConfigReader.queryLogFolder + "/sortedqueryoccurrences.log");
            ...
            ExternalSort.mergeSortedFiles(finalFilesList,new File(BenchmarkConfigReader.sortedQueriesOutputFile),
                    occurrencesComparator);
            logger.info("Process is completed successfully and the output is written to " + BenchmarkConfigReader.sortedQueriesOutputFile
                    + " and it took " + diff/1000 + " sec");

            //This call is necessary to remove all least frequent queries, and keep only the common ones
            ExternalSort.removeLeastFrequentQueries(new File(BenchmarkConfigReader.sortedQueriesOutputFile),
                    new File(BenchmarkConfigReader.readyForClusteringFile), 10);
...
//            sortedQueryOccurrencesFile.delete();
            ...
            logger.error("Exception occurred " + exp.getMessage(), exp);
        }
    }
     ...
     */
    public static void calculateQueriesSimilarities(){
        FileReader inReader;
        ...
        String line = "";
        try{
            inReader = new FileReader(BenchmarkConfigReader.readyForClusteringFile);
...
//                    arrQueries.add(_query);
                    ...
                    logger.error("Query " + line + " cannot be written into file, due to " + exp.getMessage(), exp );
                }
            }
            inReader.close();
...
//                    lStartTime = lEndTime;
            ...
            logger.error("Calculation of queries similarities failed, due to " + exp.getMessage(), exp);
        }
...
//                    arrQueries.add(URLDecoder.decode(_query, "UTF-8"));
                    ...
                    if(removeKeywords)
                        query = StringUtilities.removeKeywordsFromQuery(query);
...
//            outWriter = new FileWriter("D:/Leipzig University/AKSWBenchmark_Log/queries_IDs.txt");
            outWriter = new FileWriter(BenchmarkConfigReader.queryIDsFile);
...
//            outWriter.write(str);
    ...
    public static void makeQueryIDsWithFrequencies(String separatorLine){
        ...
        try{
            inReader = new FileReader("D:/Leipzig University/AKSWBenchmark_Log/sortedqueries.log");
            lnReader = new LineNumberReader(inReader);
...
//            }
    ...
    private static String renameVariables(String query){
        ...
        Pattern variableSplitter = Pattern.compile(variablePattern);
...
//        return d[len1 - 1][len2 - 1];
     ...
     */
    public static ArrayList<Integer> keepClusters(int leastNumberOfNodes){
        FileReader inReader;
        FileWriter outWriter;
        LineNumberReader lnReader;
        String line = "";
...
//        ArrayList<Integer> numOfNodes = new ArrayList<Integer>();
        ...
        try{
            inReader = new FileReader(BenchmarkConfigReader.clusteredQueriesInputFile);
            lnReader = new LineNumberReader(inReader);
            outWriter = new FileWriter(BenchmarkConfigReader.clusteredQueriesOutputFile);
...
//                    logger.info("Cluster number " + clusterLineParts[0] + " has been successfully written to the output file");
                    ...
                    logger.error("Cluster " + line + " cannot be written into file, due to " + exp.getMessage(), exp );
                }
            }
            inReader.close();
            ...
            logger.error("Keeping some clusters failed due to " + exp.getMessage(), exp);
        }
...
//        int median = numOfNodes.get(numOfNodes.size()/2);
     ...
     */
    public static void loadClustersToDatabase(){
        FileReader inReader;
        LineNumberReader lnReader;
        String line = "";

        try{
            inReader = new FileReader("D:/Leipzig University/AKSWBenchmark_Log/clusteredInputWithNoBrackets.txt");
            lnReader = new LineNumberReader(inReader);
             while ((line = lnReader.readLine()) != null){
                try{
                    String[] clusterLineParts = line.split("\t");
            ...
            logger.error("Uploading clusters into database failed due to " + exp.getMessage(), exp);
        }
